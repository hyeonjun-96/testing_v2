...
def initialize(arguments=None):
    ...
    if 'image_resizer.pth' in os.listdir(os.path.dirname(args.checkpoint)) and not arguments.no_resize:
        ...
        image_resizer = None
    args.tokenizer = os.path.join(path,'dataset/tokenizer.json')
...
def convert(file_path, option):
    ...
    parser.add_argument('-c', '--config', type=str, default=(os.path.join(path,'settings/config.yaml')))
    parser.add_argument('-m', '--checkpoint', type=str, default=(os.path.join(path,'checkpoints/weights.pth')))
    ...
    try:
        ...
        if option == 'latex':
            ...
            pred = call_model(args, *objs, img=img)
            mathml_output = latex2mathml.converter.convert(pred)
        ...
        pass
    except latex2mathml.exceptions.ExtraLeftOrMissingRightError:
        print("수식인식 에러(ExtraLeftOrMissingRightError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.DoubleSubscriptsError:
        print("수식인식 에러(DoubleSubscriptsError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.MissingSuperScriptOrSubscriptError:
        print("수식인식 에러(MissingSuperScriptOrSubscriptError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.DenominatorNotFoundError:
        print("수식인식 에러(DenominatorNotFoundError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.NumeratorNotFoundError:
        print("수식인식 에러(NumeratorNotFoundError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.InvalidWidthError:
        print("수식인식 에러(InvalidWidthError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.NoAvailableTokensError:
        print("수식인식 에러(NoAvailableTokensError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.InvalidStyleForGenfracError:
        print("수식인식 에러(InvalidStyleForGenfracError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.MissingEndError:
        print("수식인식 에러(MissingEndError)가 발생하였습니다.")
        pass
    except latex2mathml.exceptions.InvalidAlignmentError:
        print("수식인식 에러(InvalidAlignmentError)가 발생하였습니다.")
        pass
    args.file = None
